...
* - **Deployment**
  - {ref}`Configure high availability <how_to_configure_high_availability>` • {ref}`Control backend routing <how_to_control_haproxy_route_relation_data>` • {ref}`Configure OpenStack virtual IP <how_to_configure_virtual_ip_on_openstack>`
* - **Integrations**
...
* - **Supported protocols** 
  - {ref}`FTP <tutorial_loadbalancing_for_an_ftp_server>` • {ref}`gRPC <tutorial_loadbalancing_for_a_grpc_server>` • {ref}`HTTP/2 <reference_http2_support>`
* - **Security**
