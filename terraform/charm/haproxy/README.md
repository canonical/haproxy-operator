...
| <a name="input_machines"></a> [machines](#input\_machines) | Machines to deploy haproxy units to. Mutually exclusive with units. | `set(string)` | `null` | no |
...
| <a name="input_units"></a> [units](#input\_units) | Number of haproxy units. If hacluster is enabled, it is recommended to use a value > 3 to ensure a quorum. | `number` | `null` | no |
