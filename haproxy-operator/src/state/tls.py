...
class PeerCertificatesNotReadyError(Exception):
    """Exception raised when peer certificate data is not yet available from the leader."""
...
class TLSInformation:
    ...
    def from_charm(
        ...
        Raises:
            ...
            PeerCertificatesNotReadyError: When non-leader and peer data not yet available.
    ...
    def _from_peer_relation(
        ...
        Raises:
            PeerCertificatesNotReadyError: When peer relation or certificate data is not available.
        ...
        if not peer_relation:
            raise PeerCertificatesNotReadyError(
                "Peer relation not available, cannot read TLS data."
            )
        raw = peer_relation.data[charm.app].get(PEER_RELATION_TLS_CERT_DATABAG_KEY)
        if not raw:
            raise PeerCertificatesNotReadyError(
                "No TLS certificate data in peer relation yet."
            )
        private_key = certificates.private_key
        if not private_key:
            raise PeerCertificatesNotReadyError(
                "Private key not yet available from TLS library."
            )
        data = json.loads(raw)
